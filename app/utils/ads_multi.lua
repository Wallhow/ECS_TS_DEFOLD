local ____exports = {}
____exports.config = {is_auto_init = true, banner_interval = 30, is_banner_visible_on_init = true}
...
local banner_visible = ____exports.config.is_banner_visible_on_init
local banner_pos = -1
...
local function _start_refresh_banner(time)
    ...
    _clear_banner_timer()
    local ____timer_delay_1 = timer.delay
    local ____temp_0
    if time ~= -1 then
        ____temp_0 = time
    else
        ____temp_0 = ____exports.config.banner_interval
    end
    id_timer_banner = ____timer_delay_1(____temp_0, false, _update_banner)
end
...
local function listener(____self, message_id, message)
    ...
    if message_id == yandexads.MSG_ADS_INITED then
        if event == yandexads.EVENT_LOADED then
            ...
            is_ready = true
            if ____exports.config.is_auto_init then
                if ____exports.config.is_banner_visible_on_init and #id_banners > 0 then
                    _load_banner()
                end
                if #id_inters > 0 then
                    _load_interstitial()
    ...
    elseif message_id == yandexads.MSG_BANNER then
        if event == yandexads.EVENT_LOADED then
            ...
            banner_index = 0
            if banner_visible then
                local ____yandexads_show_banner_3 = yandexads.show_banner
                local ____temp_2
                if banner_pos == -1 then
                    ____temp_2 = yandexads.POS_BOTTOM_CENTER
                else
                    ____temp_2 = banner_pos
                end
                ____yandexads_show_banner_3(____temp_2)
            else
...
function ____exports.init(_id_banners, _id_inters, _id_rewards)
    ...
    banner_visible = ____exports.config.is_banner_visible_on_init
...
function ____exports.show_banner(pos)
    ...
    local ____temp_4
    if pos == -1 then
        ____temp_4 = yandexads.POS_BOTTOM_CENTER
    else
        ____temp_4 = pos
    end
    banner_pos = ____temp_4
    ...
    yandexads.show_banner(pos)
    print("yandexads: show_banner")
    return true
end
