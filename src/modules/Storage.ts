...
function StorageModule(file_key = 'sys_save_load') {
    ...
    let is_platform_storage = false; // todo отключил
    ...
    function apply_custom_key() {
        file_key = "sys_save_load_" + sys.get_config_string("android.package");
    }

    function set_data(key: string, tab: { value: any }) {
       /*  if (_is_html5_storage && is_platform_storage)
            return HtmlBridge.set_data_to_storage({ key, value: tab.value }, () => { }); */
    ...
    function get_data(key: string) {
      /*   if (_is_html5_storage && is_platform_storage) {
            const val = HtmlBridge.get_cached_key(key);
            if (val == null)
                return null;
            return { value: val };
        }
