...
export function newShrinkText(labelNode: node, shrinkAxis: 'x' | 'y', isAnimate = true): ShrinkText {
...
function _createShrinkText(labelNode: node, shrinkAxis: 'x' | 'y', isAnimate = true) {
    ...
    function _setScale(scale: number, isAnimate: boolean) {
        if (isAnimate)
            tween(labelNode, 'gui').to(0.2, 'scale', { scale: vmath.vector3(scale, scale, 0) }, { easing: 'EASING_OUTSINE' }).start();
        else
