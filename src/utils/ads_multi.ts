...
export const config = {
    is_auto_init: true,
    banner_interval: 30,
    is_banner_visible_on_init: true
};
...
let banner_visible = config.is_banner_visible_on_init;
let banner_pos = -1;
...
function listener(self: any, message_id: string | hash, message: any) {
    ...
    if (message_id == yandexads.MSG_ADS_INITED) {
        if (event == yandexads.EVENT_LOADED) {
            ...
            if (config.is_auto_init) {
                ...
                if (config.is_banner_visible_on_init && id_banners.length > 0)
                    _load_banner();
    ...
    // banner
        ...
        if (event == yandexads.EVENT_LOADED) {
            ...
            banner_index = 0;
            if (banner_visible)
                yandexads.show_banner(banner_pos == -1 ? yandexads.POS_BOTTOM_CENTER : banner_pos);
            else
...
export function init(_id_banners: string[] = [], _id_inters: string[] = [], _id_rewards: string[] = []) {
    if (!yandexads) {
    ...
    banner_visible = config.is_banner_visible_on_init;
...
export function show_interstitial(time = 3 * 60, first_delay = 0) {
